...
Если мы работаем с файлом, который уже добавлен в репозиторий, то можно не прописывать git add каждый раз, а использовать команду `git commit -a -m "комментарий к коммиту"
...
Гит позволяет вернуться к состоянию на момент определенного коммита. Это делается командой checkout, она также может быть использована для переключения между ветками.
`
git checkout
`
...
Для перемещения к родительским коммитам можно не использовать идентификаторы коммитов, а использовать команды типа:
`git checkout head^` эта команда перенесет на 1 коммит назад (^^ перенесёт на 2),
`git checkout head~n` эта команда перенесёт на n коммитов назад.
...
При помощи команды `git checkout -b <name_new_branch> можно создать новую ветку и сразу перейти на неё.
...
`git clone
https://github.com/somebody/awesome-project new-folder`
