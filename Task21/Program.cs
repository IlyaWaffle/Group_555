...
// Задаем переменные по осям X Y Z
int A_x, A_y, A_z, B_x, B_y, B_z;

// Переводим ввод строки в целые числа x y z
Console.Write($"Введите координату x в точке A: ");

// говорим что они целове.пробуем перевестию. читаем с линии. выводим в переменную 
...
// вычитаем стороны B и А потом умножаем результат
int x = (B_x - A_x) * (B_x - A_x);
int y = (B_y - A_y) * (B_y - A_y);
int z = (B_z - A_z) * (B_z - A_z);

// складываем все рузультаты
int xyz = x + y + z;

// Round(Double) Округляет значение с плавающей запятой двойной точности до ближайшего целого значения; 
